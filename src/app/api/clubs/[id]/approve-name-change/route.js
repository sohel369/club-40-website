...
export async function POST(req, { params }) {
  ...
  try {
    if (authResult.user.role !== 'super_admin') {
      return NextResponse.json({ message: 'Only Super Admins can approve club name changes!' }, { status: 403 });
    }
    ...
    if (isUsingMockDb) {
      ...
      club.name = { en: club.nameChangeRequest.requestedName.en, bn: club.nameChangeRequest.requestedName.bn };
    ...
    console.error(err);
    return NextResponse.json({ message: 'Server error approving name change' }, { status: 500 });
  }
}